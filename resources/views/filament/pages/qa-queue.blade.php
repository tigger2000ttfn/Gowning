        ...
        <div class="gqs-panel-body">
            @forelse ($queue as $q)
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 16px;border-bottom:1px solid var(--gqs-border,#F2F2F4);">
                    <div>
                        <div style="font-weight:700;color:var(--gqs-text,#1A1A1F);"><button type="button" wire:click="$dispatch('open-qual-modal', { id: {{ $q->id }} })" style="background:none;border:none;padding:0;cursor:pointer;color:var(--gqs-text,#1A1A1F);font-weight:700;text-decoration:underline;text-decoration-style:dotted;text-underline-offset:2px;" title="Open record">{{ $q->personnel?->full_name ?? 'Unknown' }}</button></div>
