...
@section('body')
    ...
    <div class="sec">
        ...
        @if($overdue->isEmpty())<div class="empty">None overdue.</div>@else
            <table class="data"><thead><tr><th>Employee</th><th>Name</th><th>Due Date</th></tr></thead><tbody>
                @foreach($overdue as $q)<tr><td>{{ $q->personnel?->employee_id }}</td><td>{{ $q->personnel?->full_name }}</td><td><span class="pill pill-red">{{ $q->due_date?->format('d M Y') }}</span></td></tr>@endforeach
            </tbody></table>
        @endif
    </div>
    ...
    <div class="sec">
        ...
        @if($upcoming->isEmpty())<div class="empty">None upcoming.</div>@else
            <table class="data"><thead><tr><th>Employee</th><th>Name</th><th>Due Date</th></tr></thead><tbody>
                @foreach($upcoming as $q)<tr><td>{{ $q->personnel?->employee_id }}</td><td>{{ $q->personnel?->full_name }}</td><td><span class="pill pill-gold">{{ $q->due_date?->format('d M Y') }}</span></td></tr>@endforeach
            </tbody></table>
        @endif
    </div>
