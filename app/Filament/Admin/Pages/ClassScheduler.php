...
class ClassScheduler extends Page
{
    ...
    protected function enrollPersonIntoSession(int $sessionId): void
    {
        ...
        Notification::make()->success()->title('Scheduled')
            ->body($p->full_name . ' booked into ' . ($session->trainingClass?->name ?? 'class') . ' on ' . $session->session_date?->gmp() . '.')->send();
    }

    // reusable in-app confirmation modal (no native system prompts)
    public array $confirm = [];

    /** Deferred class attendance intent per enrollment: 'attended' | 'no_show'. Committed on submit. */
    public array $attIntent = [];
    ...
    public function confirmSubmitSign(): void
    {
        ...
        \App\Models\ElectronicSignature::create([
            ...
            'statement' => 'Attendance submitted for ' . $attendedCount
    ...
    {
        if (! in_array($status, ['attended', 'no_show'], true)) return;
    ...
    protected function moveEnrollment(?int $enrollmentId, int $sessionId): void
    {
        ...
        Notification::make()->success()->title('Rescheduled')
            ->body($e->name . ' moved to ' . ($target->trainingClass?->name ?? 'class') . ' on ' . $target->session_date?->gmp() . '.')->send();
    }
    ...
    protected function moveEnrollmentToNextOpen(?int $enrollmentId): void
    {
        ...
        $e->save();
        Notification::make()->success()->title('Rescheduled')
            ->body($e->name . ' moved to ' . ($next->trainingClass?->name ?? 'class') . ' on ' . $next->session_date?->gmp() . '.')->send();
    }
